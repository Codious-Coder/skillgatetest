  ...
  <div class="flex min-h-full items-center justify-center p-6">
    <div class="w-full max-w-md">
      <div class="mb-8 text-center">
        <div class="mx-auto mb-4 flex h-12 w-12 items-center justify-center rounded-2xl bg-indigo-500/20 ring-1 ring-indigo-400/30">
          <!-- Simple icon -->
          <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 text-indigo-300" viewBox="0 0 24 24" fill="none" stroke="currentColor">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M16.5 10.5V6.75A4.5 4.5 0 0 0 12 2.25v0a4.5 4.5 0 0 0-4.5 4.5V10.5M5.25 21.75h13.5a1.5 1.5 0 0 0 1.5-1.5v-6.75a1.5 1.5 0 0 0-1.5-1.5H5.25a1.5 1.5 0 0 0-1.5 1.5v6.75a1.5 1.5 0 0 0 1.5 1.5Z"/>
          </svg>
        </div>
      ...
      <div class="glass rounded-2xl p-6 shadow-2xl">
        <form method="POST" action="{{ route('login') }}" class="space-y-4">
          ...
          <div class="flex items-center justify-between">
            <label class="inline-flex items-center gap-2 text-sm text-slate-300 select-none">
              <input type="checkbox" name="remember" class="h-4 w-4 rounded border-white/20 bg-white/5 text-indigo-500 focus:ring-indigo-400/50">
              Remember me
            </label>
            <a href="{{ '#' }}" class="text-sm text-indigo-300 hover:text-indigo-200">Forgot Password?</a>
          </div>
          ...
          <button type="submit"
            ...
            <span>Sign in</span>
            <svg class="h-4 w-4 transition group-hover:translate-x-0.5" viewBox="0 0 24 24" fill="none" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.6" d="M5 12h14M13 5l7 7-7 7"/>
            </svg>
          </button>
        </form>
        ...
        <div class="grid grid-cols-1 gap-3">
          <button class="w-full rounded-xl border border-white/15 bg-white/5 px-4 py-2.5 text-sm text-slate-200 hover:bg-white/10"><a href="{{ '#' }}">Examinee? Sign in with OTP</button>
        </div>
      </div>

      <p class="mt-6 text-center text-xs text-slate-400">
        By continuing you agree to our <a href="#" class="underline decoration-white/30 hover:text-slate-200">Terms</a> and <a href="#" class="underline decoration-white/30 hover:text-slate-200">Privacy Policy</a>.
      </p>
    </div>
  </div>
</body>
